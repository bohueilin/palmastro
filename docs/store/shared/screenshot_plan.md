...
| 3 | Dashboard | `ui/results/ResultsScreen.kt` — four domain cards | Demo data: 4 domains with score/grade/confidence + month theme | Four life areas, scored and explained | 四大生活領域，每個分數都有依據 |
...
| 8 | Paid pack | **N/A at launch** — free-only launch, no paid UI exists | Substitute: `ui/history/HistoryScreen.kt` monthly delta view | Rescan monthly and watch the change | 每月重新掃描，看見自己的變化 |
...
screenshot would misrepresent the app to both stores. The History/monthly-delta shot
covers the PRD §34.2 delta-tracking differentiator instead. Revisit when IAP ships.
...
- Play + App Store order: 3, 5, 2, 7, 4, 8, 6, 1 — lead with dashboard and
  explainability (differentiators), keep onboarding last.
  *(Assumption (editable): PRD fixes the set, not the order.)*

## App preview video (optional, PRD §67)

If produced: 15–25s silent-friendly capture showing scan → results → explainability →
privacy settings, no overclaiming text, no fast flashing (PRD §41 motion rules,
