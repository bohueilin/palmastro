...
interface MonthlyResultDao {
    ...
    // Rescan safety: if duplicate rows ever exist for a month, the NEWEST scan wins.
    @Query("SELECT * FROM monthly_result WHERE monthKey = :monthKey ORDER BY createdAt DESC LIMIT 1")
    ...
    @Query("DELETE FROM monthly_result WHERE monthKey = :monthKey")
